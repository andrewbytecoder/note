...
- 性能指标 ^9125b8d5-1973-6772
	...
	- 上下文切换
	（无损耗）
		- 如果系统的上下文切换次数比较稳定，那么理想数据是１万以内。 ^69717463-9252-f9c3
			- 相关工具
				- vmstat
					- cs列：系统每秒上下文切换次数
					- r列：处于可运行状态的进程数量
...
- 工具汇总 ^bd15351c-478c-5065
	- top ^8c0a8f3d-7ffb-5d47
	...
	- pidstat ^748339a1-d587-0ceb
		...
		%wait: 进程等待运行时所花费的CPU时间
			...
			- -v：进程相关的线程数和文件描述符数量
			- -s：进程stack锁用内存信息
	- perf ^805750da-7988-1718
		- 用法 ^d9aaa53a-c5cb-125d
	...
	- pstree
		...
		显示5638这个进程的进程树（包含线程）
		$ pstree -T -p 5638
		显示5638这个进程的进程树（不包含线程） ^6f8dcbc6-74e7-2299
			- -a：显示命令行参数
		...
		- 物理层
			- 透明传输比特流
				- 常见硬件厂商
					...
					- intel
	- linux网络设备驱动
	（以dm9000芯片为例） ^8f509093-dbe1-c58f
		- 硬件datasheet
		...
		- 软中断NET_TX,NET_RX处理
			...
			- 网络底半部接收
				- net_rx_action()
- 性能指标 ^ba6ceb2d-aecb-5509
	- 最大传输单元mtu ^39c4c56f-bd91-eb37
		- 相关工具
			- 查看
