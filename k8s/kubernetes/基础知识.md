...
查看 `kubectl get pod -n base-services grafana-6db6b7cc55-nfzn8 -o yaml`
...
metadata:
  ...
  - apiVersion: apps/v1
    ...
    controller: true
    kind: ReplicaSet
    name: grafana-6db6b7cc55
    # 我来自谁， 是谁创建了这个pod，是ReplicaSet
    uid: e3532491-a189-40f6-9807-80f97d24f66a
  resourceVersion: "775468"
  # 这个uid才是真正这个pod的uid，后面查询pod配置也是使用这个uid进行
  uid: 022cbff6-abac-419a-8922-065db7709efc
```

在创建对应目录的时候会将 `-` 转化为 `_`， 因此如果想找到对应容器的cgroup目录需要将对应的uid转换一下才行

当知道对应uid目录中设置的内存等信息出现问题时，可以通过如下命令获取出对应的pod是哪个
```bash
# 获取所有 Pod 的 NAME、NAMESPACE、UID 
kubectl get pods --all-namespaces -o custom-columns=NAMESPACE:.metadata.namespace,NAME:.metadata.name,UID:.metadata.uid | grep 211315b6
```

> **`211315b6-4b49-4482-9526-2794e3794fe9` 就是该 Pod 的 `metadata.uid`（UUID）**，只是把 `-` 替换成了 `_` 并加了前缀 `pod`。

k8s在目录 `/sys/fs/cgroup/kubepods.slice` 中对内存和cpu通过cgroup进行管理
```bash
# 目录 `/sys/fs/cgroup/kubepods.slice` 中存放的是整个系统的内存总量限制
/sys/fs/cgroup/kubepods.slice
```

除了总目录下的，会根据limits配置的不同，划分为不同的子目录

- `kubepods-besteffort.slice`
- `kubepods-burstable.slice`
- `kubepods-guaranteed.slice`
 
是 **由 Pod 的资源请求（requests）和限制（limits）配置决定的**。这是 Kubelet 实现 **QoS（Quality of Service，服务质量）分级** 的核心机制。


### 三种 QoS 类别及其 cgroup 分配

Kubernetes 根据容器的 `resources.requests` 和 `resources.limits` 将 Pod 分为三类 QoS：

|QoS Class|条件|cgroup slice 路径示例|
|---|---|---|
|**Guaranteed**|所有容器都设置了 **CPU 和内存的 requests = limits**（且不为 0）|`kubepods-guaranteed.slice`|
|**Burstable**|不满足 Guaranteed，但 **至少有一个容器设置了 requests 或 limits**|`kubepods-burstable.slice`|
|**BestEffort**|所有容器都 **未设置任何 requests 和 limits**|`kubepods-besteffort.slice`|
...
- 某个容器只设了 `requests`，没设 `limits`
- `requests` ≠ `limits`
- 只设了 `limits`，没设 `requests`（K8s 会自动将 `requests` 默认为 `limits`，但仍属于 Burstable）
- 部分容器有资源设置，部分没有

```yaml
# 示例：Burstable（requests < limits）
resources:
  requests:
    memory: "256Mi"
  limits:
...
```yaml
# 示例：Burstable（只设 limits）
resources:
  limits:
    memory: "512Mi"
# → K8s 自动设 requests=limits，但 QoS 仍是 Burstable！
```

> ⚠️ 注意：**即使所有容器都设置了 limits，只要 requests ≠ limits（或未显式设置 requests），就是 Burstable**。
